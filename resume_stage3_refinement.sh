...
if env.get('weather_source') != 'random':
    raise SystemExit('Refinement requires weather_source=random, got %r' % env.get('weather_source'))
...
Same Stage 3 full-year random-weather training contract, but with a larger
total epoch budget so a restored checkpoint can continue optimizing.
"""

import importlib

_base = importlib.import_module('examples.config.pv_tracking.stage3_fullyear_random')
...
echo "  (full year, random weather, physical observations, same reward/action)."
