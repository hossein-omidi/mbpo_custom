...
Only after **F** passes → Stage 2 (random weather). Stage 2/3 configs are **not** defined in this document.
...
Stage 2  →  random weather (define new config + NEW eval dir name)
...
| Step | Stage 0 | Stage 1 |
|------|---------|---------|
| Config | `stage0_single_day` | `0` |
| Eval dir | `evaluation/pv_stage0_single_day` | `evaluation/pv_stage1_clearsky_summer` |
| Fixed dates | `2020-06-21` | four summer dates in `0.py` |
| Gate command | `diagnose_tracking.py --eval-dir evaluation/pv_stage0_single_day --gate` | same with `pv_stage1_clearsky_summer` |
