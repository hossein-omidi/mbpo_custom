...
def main():
    ...
    for key, (date, wsrc) in scenarios.items():
        ...
        env.close()
        report.append('  sun track energy=%.4f kWh  fixed=%.4f kWh' % (
    ...
    # --- Intra-day state variation by day type ---
    day_keys = {
    ...
    # Per-feature std ratio cloudy/clear (weather-dominated features should drop on cloudy day)
    ...
    # --- Power effect % by state factor (pvlib OAT sensitivity) ---
    oat_pct_by_label = {}
    corr_paths = []
