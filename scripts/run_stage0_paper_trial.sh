#!/usr/bin/env bash
# Stage 0 MBPO paper (stage0_single_day_mbpo_paper.py) — train / plot / eval / gate.
set -euo pipefail
...
EVAL_DIR="$ROOT/evaluation/pv_stage0_mbpo_paper"
...
for t in sorted(glob.glob(os.path.join(root, 'seed:*')), key=os.path.getmtime, reverse=True):
    ...
    if os.path.isfile(p) and want in json.load(open(p)).get('config_version', ''):
...
case "$CMD" in
  train)
    ...
    echo "=== MBPO PAPER | cpus=$CPUS trial-cpus=$TRIAL_CPUS | $RAY_TMP ==="
